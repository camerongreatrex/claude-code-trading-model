...
Usage:
  ...
  python paper_trader.py run|eod   — EOD update (after 4:30 PM ET)
  python paper_trader.py catchup   — replay missed EOD days
...
from v1.config.params import LIVE_REBALANCE_MODE, LIVE_REBALANCE_DAYS, LIVE_TOP_N, LIVE_MIN_HOLD_DAYS
...
STRATEGIES = {
    ...
    "top11_adx22_momt_ac55_cap1": {
        "signal_col": "signal_multi", "sizing": "topn_v2",
        "mom_tilt": False, "pca_scale": False, "dd_control": False, "macro": False,
        "rebalance_mode": LIVE_REBALANCE_MODE, "top_n": LIVE_TOP_N,
    },
...
# Instances that must stay current without the dashboard open (scheduler / Actions).
ALL_PT_INSTANCES = ("", "v4nf_3mo")


@contextmanager
def use_pt_instance(instance: str):
    """Temporarily repoint module-level I/O paths to another paper-trading folder."""
    global _PT_INSTANCE, PT_DIR, STATE_FILE, TRADES_FILE, HISTORY_FILE
    new_dir = (Path("data/v1/paper_trading") if not instance
               else Path(f"data/v1/paper_trading_{instance}"))
    new_dir.mkdir(parents=True, exist_ok=True)
    old = {
        "_PT_INSTANCE": _PT_INSTANCE,
        "PT_DIR": PT_DIR,
        "STATE_FILE": STATE_FILE,
        "TRADES_FILE": TRADES_FILE,
        "HISTORY_FILE": HISTORY_FILE,
    }
    _PT_INSTANCE = instance
    PT_DIR = new_dir
    STATE_FILE = new_dir / "state.json"
    TRADES_FILE = new_dir / "trades.csv"
    HISTORY_FILE = new_dir / "history.csv"
    try:
        yield
    finally:
        _PT_INSTANCE = old["_PT_INSTANCE"]
        PT_DIR = old["PT_DIR"]
        STATE_FILE = old["STATE_FILE"]
        TRADES_FILE = old["TRADES_FILE"]
        HISTORY_FILE = old["HISTORY_FILE"]
...
def _compute_topn_v2_targets(
    pv: float, signals: dict,
    top_n: int | None = None, target_vol: float = 0.14, scale_max: float = 2.5,
    vt_window: int = 63, lev_x: float = 1.5, max_gross: float = 1.0,
...
def _rebalance_to_targets(state: dict, targets: dict, prices: dict,
    ...
    """
    Drive state to match the sizer's target dollar dict.

    Not used in live paper trading (top-up/trim removed). Kept for backtest
    cadence studies via portfolio.apply_live_cadence().
...
def _resolve_rebalance_mode(strat_cfg: dict) -> str:
    """Live execution is signal-only (entry/exit). Daily/monthly topup removed."""
    return "signal_only"


def _position_hold_days(pos: dict, trade_date: str) -> int:
    """Weekdays since entry (inclusive of entry day = 0)."""
    ed = pos.get("entry_date", "")
    if not ed:
        return LIVE_MIN_HOLD_DAYS
    d0 = date.fromisoformat(str(ed)[:10])
    d1 = date.fromisoformat(trade_date)
    if d1 <= d0:
        return 0
    return len(_trading_days_inclusive(d0 + timedelta(days=1), d1))


def _apply_safe_haven_floor(
    state: dict,
    signals: dict,
    prices: dict,
    trade_date: str,
    entry_reason: str = "safe_haven_floor",
    target_set: set | None = None,
) -> dict:
    """Enter floor allocations when flat; exit on short override. No top-up."""
    pv = _portfolio_value(state, prices)
    for _floor_ticker, _floor_frac in _get_current_safe_haven_floor().items():
        if target_set and _floor_ticker in target_set:
            continue
        ...
        _raw_sig     = signals[_floor_ticker].get(
            "raw_signal", signals[_floor_ticker]["signal"],
        )

        if _raw_sig == -1:
            if _floor_ticker in state["positions"]:
                state = _sell(state, _floor_ticker, _floor_price,
                              reason="floor_short_override", trade_date=trade_date)
        elif (_floor_ticker not in state["positions"]
              and signals[_floor_ticker]["signal"] == 0):
            if state["cash"] >= _floor_size * 1.01:
                state = _buy(state, _floor_ticker, _floor_price, _floor_size,
                             reason=entry_reason, trade_date=trade_date)
    return state


def _run_signal_eod_trades(
    state: dict,
    signals: dict,
    prices: dict,
    trade_date: str,
    ...
    *,
    ...
    entry_reason: str = "signal_entry",
    skip_pca: bool = False,
    skip_macro: bool = False,
) -> dict:
    """
    Medium-frequency live execution (no top-up/trim).

    Top-N strategies (topn_v2 / topn_vt):
      - Enter new top-N names at sizer size when signal==1
      - Exit on signal==0, or rank_exit when dropped from top-N after min hold

    Other strategies: classic signal==1 entry / signal==0 exit.
    """
    sizing = strat_cfg.get("sizing", "atr")
    pv = _portfolio_value(state, prices)
    topn_vt_targets = (
        _compute_topn_vt_targets(pv, signals)
        if sizing == "topn_vt" else None
    )
    _top_n = int(strat_cfg.get("top_n", LIVE_TOP_N))
    topn_v2_targets = (
        _compute_topn_v2_targets(pv, signals, top_n=_top_n)
        if sizing == "topn_v2" else None
    )
    pf_targets = topn_v2_targets if topn_v2_targets is not None else topn_vt_targets
    use_rank = pf_targets is not None

    target_set: set[str] = set()
    if pf_targets:
        _floor_keys = set(_get_current_safe_haven_floor().keys())
        pf_targets = {
            t: v for t, v in pf_targets.items()
            if not (t in _floor_keys
                    and signals.get(t, {}).get("raw_signal",
                          signals.get(t, {}).get("signal", 0)) == -1)
        }
        target_set = {t for t, v in pf_targets.items() if v > 0}

    for ticker in list(state["positions"]):
        if ticker.startswith("_") or ticker not in signals:
            continue
        sig = signals[ticker]
        raw = sig.get("raw_signal", sig.get("signal", 0))
        px = prices.get(ticker)
        if not px or float(px) <= 0:
            continue
        px = float(px)
        ...
        if sig["signal"] == 0:
            if ticker in SAFE_HAVEN_FLOOR and raw == 0:
                continue
            state = _sell(state, ticker, px, reason=exit_reason, trade_date=trade_date)
            continue

        if use_rank and ticker not in target_set:
            if _position_hold_days(pos, trade_date) >= LIVE_MIN_HOLD_DAYS:
                reason = "rank_exit" if exit_reason == "signal_exit" else exit_reason
                state = _sell(state, ticker, px, reason=reason, trade_date=trade_date)

    pv = _portfolio_value(state, prices)
    entry_candidates = [
        t for t in TICKER_LIST
        if t not in state["positions"] and t in signals
        and signals[t]["signal"] == 1
    ]

    rp_weights = _compute_rp_weights(state, signals) if sizing == "rp" else {}
    mom_rank = {}
    ...
    pca_s   = 1.0 if skip_pca else (_pca_scale(state, signals) if strat_cfg.get("pca_scale") else 1.0)
    dd_s    = _drawdown_scale(state) if strat_cfg.get("dd_control") else 1.0
    macro_m = 1.0 if skip_macro else (_macro_live_multiplier() if strat_cfg.get("macro") else 1.0)
    ...
    if topn_v2_targets is not None:
        print(f"  Top-N V2 (ADX22+momt+AC55): {len(topn_v2_targets)} names selected, "
              f"target gross ${sum(topn_v2_targets.values()):,.0f}  (top_n={_top_n})")
        print(f"  Execution: rank_rotate (min_hold={LIVE_MIN_HOLD_DAYS}d, no top-up)")
    ...
    for ticker in entry_candidates:
        ...
        if size <= 0:
            continue
        px = prices.get(ticker, sig["close"])
        if state["cash"] >= size * 1.01:
            state = _buy(state, ticker, float(px), size,
                         reason=entry_reason, trade_date=trade_date)

    return _apply_safe_haven_floor(
        state, signals, prices, trade_date, target_set=target_set or None,
    )


def _trading_days_since(from_str: str, to_str: str) -> int:
    """Count weekdays between two ISO dates (exclusive of endpoints)."""
    if not from_str:
        return LIVE_REBALANCE_DAYS + 1
    return len(_trading_days_between(date.fromisoformat(from_str),
                                    date.fromisoformat(to_str)))


def _apply_topn_targets(
    state: dict,
    targets: dict,
    prices: dict,
    today_str: str,
    ...
    """
    Apply top-N portfolio targets with configurable rebalance cadence.

    Not used in live paper trading — live uses _run_signal_eod_trades() instead.

    signal_only (default): enter new top-N names; exit on rank drop or
    signal==0; no daily trim/topup (cuts churn vs _rebalance_to_targets).

    monthly: full rebalance every LIVE_REBALANCE_DAYS trading days.

    daily: delegate to _rebalance_to_targets (legacy high-frequency).
    """
    mode = _resolve_rebalance_mode(strat_cfg)
    if mode == "daily":
        return _rebalance_to_targets(state, targets, prices, today_str)

    if mode == "monthly":
        last_rb = state.get("last_rebalance_date", "")
        if _trading_days_since(last_rb, today_str) < LIVE_REBALANCE_DAYS:
            print(f"  Rebalance: monthly hold ({last_rb or 'init'} → {today_str})")
            return state
        state = _rebalance_to_targets(state, targets, prices, today_str)
        state["last_rebalance_date"] = today_str
        return state

    # signal_only: entries/exits only; hold size until exit
    floor_set = set(_get_current_safe_haven_floor().keys())
    target_set = {t for t, v in targets.items() if v > 0}
    ...
    for ticker in list(state["positions"]):
        ...
        sig = signals[ticker]
        raw = sig.get("raw_signal", sig.get("signal", 0))
        if ticker in floor_set and raw == 0:
            continue
        # Still in sizer targets (incl. diversifier sleeve) — hold; no daily trim.
        if ticker in target_set:
            continue
        px = prices.get(ticker)
        if not px or float(px) <= 0:
            continue
        reason = "rank_exit" if ticker not in target_set else "signal_exit"
        state = _sell(state, ticker, float(px), reason=reason, trade_date=today_str)

    for ticker, tgt in sorted(targets.items(), key=lambda x: -x[1]):
        if tgt <= 0 or ticker in state["positions"]:
            continue
        if ticker not in signals or signals[ticker].get("signal", 0) != 1:
            continue
        px = prices.get(ticker)
        if not px or float(px) <= 0:
            continue
        if state["cash"] >= tgt * 1.01:
            state = _buy(state, ticker, float(px), tgt,
...
def init_positions():
    ...
    state = _apply_safe_haven_floor(
        state, signals, prices, today_str, entry_reason="init_safe_haven_floor",
    )
...
def end_of_day_update():
    ...
    # ── Signal entries / exits (no daily top-up or trim) ───────────────────────
    state = _run_signal_eod_trades(state, signals, prices, today_str, strat_cfg)
...
def _end_of_day_update_for_date(as_of: date) -> None:
    ...
    state = _run_signal_eod_trades(
        state, signals, prices, as_of_str, _scfg,
        exit_reason="catchup_signal_exit",
        entry_reason="catchup_signal_entry",
        skip_pca=True,
        skip_macro=True,
    )
...
def _trading_days_inclusive(start: date, end: date) -> list:
    """Return all weekdays from start through end (inclusive)."""
    days, cursor = [], start
    while cursor <= end:
        if cursor.weekday() < 5:
            days.append(cursor)
...
def _missing_history_dates() -> list:
    """Weekdays with no history.csv row between two recorded EOD dates."""
    hist = load_history()
    if len(hist) < 2:
        return []
    dates = sorted({pd.Timestamp(d).date() for d in hist["date"]})
    missing = []
    for i in range(len(dates) - 1):
        missing.extend(_trading_days_between(dates[i], dates[i + 1]))
    return sorted(set(missing))


def _rebuild_state_from_trades(until: str, base_state: dict) -> dict:
    """Rebuild cash/positions by replaying trades.csv through ``until`` (inclusive)."""
    trades = load_trades()
    if trades.empty:
        return base_state

    trades = trades[trades["date"] <= pd.Timestamp(until)].sort_values("date")

    # Cash ledger: sum trade flows (matches history.csv cash at EOD anchors).
    cash = float(base_state.get("initial_capital", INITIAL_CAPITAL))
    ...
    for _, row in trades.iterrows():
        ...
        if action == "BUY":
            if ticker in positions and reason == "rebalance_topup":
                pos = positions[ticker]
                pos["shares"] += shares
                pos["cost_basis"] = pos.get("cost_basis", 0.0) + value
            ...
            pos = positions[ticker]
            is_trim = reason == "rebalance_trim"
            if not is_trim or shares < pos["shares"] * 0.9999:
                if is_trim:
                    cb_share = pos.get("cost_basis", 0.0) * (shares / pos["shares"])
                    pos["shares"] -= shares
                    pos["cost_basis"] = max(0.0, pos.get("cost_basis", 0.0) - cb_share)
                    if pos["shares"] < 1e-6:
    ...
    hist = load_history()
    hist_row = hist[hist["date"] == pd.Timestamp(until)]
    if not hist_row.empty:
        cash = float(hist_row.iloc[-1]["cash"])
        pv   = float(hist_row.iloc[-1]["portfolio_value"])
    else:
        pv = cash
    ...
    return {
        "cash": cash,
        "positions": positions,
        "initial_capital": float(base_state.get("initial_capital", INITIAL_CAPITAL)),
        "initialized_date": base_state.get("initialized_date", until),
        "last_eod_date": until,
        "portfolio_value": pv,
        "strategy": base_state.get("strategy", _best_oos_strategy()),
    }


def _truncate_ledger_after(anchor: str) -> None:
    """Keep only history/trades rows on or before ``anchor`` (YYYY-MM-DD)."""
    anchor_ts = pd.Timestamp(anchor)
    for path in (HISTORY_FILE, TRADES_FILE):
        if not path.exists():
            continue
        df = pd.read_csv(path)
        df["date"] = pd.to_datetime(df["date"])
        df[df["date"] <= anchor_ts].to_csv(path, index=False)


def _catchup_rewind_and_replay(anchor: date, days_to_replay: list) -> int:
    """
    Rewind state/ledger to ``anchor``, then replay each date in ``days_to_replay``.
    Backs up state.json, history.csv, and trades.csv before mutating.
    """
    import shutil

    anchor_str = str(anchor)
    days_to_replay = sorted({d for d in days_to_replay if d > anchor})
    if not days_to_replay:
        return 0

    print(f"\nGap catch-up: rewind to {anchor_str}, replay {len(days_to_replay)} day(s)")
    print(f"  Dates: {days_to_replay[0]} → {days_to_replay[-1]}\n")

    for fp in (STATE_FILE, HISTORY_FILE, TRADES_FILE):
        if fp.exists():
            bak = fp.with_suffix(fp.suffix + ".pre_catchup.bak")
            shutil.copy2(fp, bak)
            print(f"  Backup → {bak}")

    base = load_state()
    _truncate_ledger_after(anchor_str)
    state = _rebuild_state_from_trades(anchor_str, base)
    save_state(state)

    n = 0
    for d in days_to_replay:
        _end_of_day_update_for_date(d)
        n += 1

    print(f"\nGap catch-up complete — {n} day(s) replayed.\n")
    return n


def catchup(from_date: date | None = None, to_date: date | None = None,
            fill_gaps: bool = False) -> int:
    """
    Detect and replay missed trading days.

    Default (scheduler / ``catchup`` CLI): replay weekdays strictly between
    ``last_eod_date`` and today.

    Gap mode (``--fill-gaps`` or ``--from`` / ``--to``): detect holes in
    history.csv, rewind to the last good EOD before the hole, and replay
    through the former ``last_eod_date`` so later rows stay consistent.

    Args:
        from_date: Optional first date to include in a manual gap fill.
        to_date:   Optional last date to include in a manual gap fill.
        fill_gaps: When True, auto-detect missing history rows and fill them.

    Returns:
        Number of EOD days replayed.
    """
    state = load_state()
    if not state:
        print("No portfolio found.  Run 'init' (or _init_v4nf_at_open.py) first.")
        return 0

    inst = f" ({_PT_INSTANCE})" if _PT_INSTANCE else " (main)"
    print(f"Paper trading dir: {PT_DIR}{inst}")

    last_str = state.get("last_eod_date", "")
    tail_eod = date.fromisoformat(last_str) if last_str else None
    n = 0

    # ── Heal holes in history.csv (always — not only with --fill-gaps) ───────
    missing = _missing_history_dates()
    if from_date:
        missing = [d for d in missing if d >= from_date]
    if to_date:
        missing = [d for d in missing if d <= to_date]
    if from_date and to_date and not missing:
        missing = _trading_days_inclusive(from_date, to_date)

    if missing and tail_eod:
        anchor = _previous_trading_day(min(missing))
        replay_through = max(tail_eod, max(missing))
        days = _trading_days_inclusive(min(missing), replay_through)
        n = _catchup_rewind_and_replay(anchor, days)
        state = load_state()
        last_str = state.get("last_eod_date", "")

    # ── Standard tail catch-up (last_eod → today) ────────────────────────────
    if not last_str:
        return n
    ...
    if not missed:
        if n == 0:
            print("Catch-up: portfolio is up to date.")
        return n

    print(f"\nCatch-up: {len(missed)} missed trading day(s) after last EOD")
    ...
    print(f"\nCatch-up complete — {len(missed)} tail day(s) replayed.\n")
    return n + len(missed)


def catchup_all_instances(from_date: date | None = None,
                          to_date: date | None = None,
                          fill_gaps: bool = False) -> dict[str, int]:
    """
    Run catchup() for every configured paper-trading instance.

    Returns:
        Dict mapping instance label → number of EOD days replayed.
    """
    ...
    for inst in ALL_PT_INSTANCES:
        ...
        with use_pt_instance(inst):
            if not load_state():
                print(f"Catch-up skip ({label}): not initialised")
                results[label] = 0
                continue
            results[label] = catchup(
                from_date=from_date, to_date=to_date, fill_gaps=fill_gaps,
            )
    return results
...
def compute_live_portfolio_metrics() -> dict:
    ...
    # Computed BEFORE today_unrealized because the unrealized figure is derived
    # from the daily-P&L identity below and needs today_realized as an input.
    all_realized   = 0.0
    today_realized = 0.0
    realized_gains = 0.0
    realized_losses = 0.0
    ...
    # ── Today's unrealized P&L (derived from the daily-P&L identity) ─────────
    # Identity: pv − prev_pv = today_realized + today_unrealized
    # Computing today_unrealized as `tot_cur_val − prev_positions_val` was wrong
    # whenever any trades fired today: cash spent on new BUYs inflated tot_cur_val
    # without any actual P&L, and proceeds from SELLs deflated it.  Using the
    # identity guarantees the two "today" metrics always sum to the daily PV
    # change shown on the vs-S&P tab.  Buy commissions hit unrealized (not
    # realized) since they reduce PV without any sale event.
    today_unrealized     = daily_pnl_usd - today_realized
    today_unrealized_pct = (today_unrealized / prev_pv * 100
                            if prev_pv else 0.0)
...
def _print_cli_usage():
    inst = "PT_INSTANCE=v4nf_3mo " if not _PT_INSTANCE else ""
    print(
        f"\nUsage: {inst}PYTHONPATH=. python v1/scripts/paper_trader.py <command>\n"
        "\nCommands:\n"
        "  init       One-time portfolio setup\n"
        "  run, eod   End-of-day update for today (after 4:30 PM ET)\n"
        "  catchup    Replay missed EOD days (see options below)\n"
        "  status     Print current portfolio state\n"
        "  strategy   List or change active strategy\n"
        "\nCatch-up options:\n"
        "  catchup                  Days since last_eod_date → today\n"
        "  catchup --fill-gaps      Fill holes in history.csv, then tail catch-up\n"
        "  catchup --from DATE --to DATE   Fill a specific date range (YYYY-MM-DD)\n"
        "\nV4N-F 3-month instance:\n"
        "  PT_INSTANCE=v4nf_3mo PYTHONPATH=. python v1/scripts/paper_trader.py catchup --fill-gaps\n"
    )


def _parse_catchup_cli() -> int:
    args = sys.argv[2:]
    from_d = to_d = None
    fill_gaps = False
    i = 0
    while i < len(args):
        if args[i] == "--from" and i + 1 < len(args):
            from_d = date.fromisoformat(args[i + 1])
            i += 2
        elif args[i] == "--to" and i + 1 < len(args):
            to_d = date.fromisoformat(args[i + 1])
            i += 2
        elif args[i] == "--fill-gaps":
            fill_gaps = True
            i += 1
        else:
            print(f"Unknown catchup option: {args[i]}")
            _print_cli_usage()
            return 0
    return catchup(from_date=from_d, to_date=to_d, fill_gaps=fill_gaps)


def main_cli():
    """Terminal entry point (also used by paper_trader_v4nf.py)."""
    cmd = sys.argv[1] if len(sys.argv) > 1 else "status"
    handlers = {
        "init":     init_positions,
        "run":      end_of_day_update,
        "eod":      end_of_day_update,
        "catchup":  _parse_catchup_cli,
        "status":   print_status,
    ...
    if fn is None:
        ...
        return
    result = fn()
    if cmd == "catchup" and isinstance(result, int):
        return result
