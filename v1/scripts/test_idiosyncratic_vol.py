"""
Idiosyncratic-volatility sleeve on top of V4N-F base.

Hypothesis: The idiosyncratic-volatility (IVOL) anomaly — names with low
idiosyncratic risk (CAPM residual std) earn higher risk-adjusted forward
returns than high-IVOL names — is a distinct cut from total-vol low-vol.
While simple low-vol picks defensives that overlap with V4N's quality tilt,
IVOL filters out *systematic* (beta) risk and leaves only stock-specific
noise.  This may catch defensive names that V4N's beta-tilted trend signals
ignore.

Mechanism: rolling 60d/120d CAPM regression vs SPY → take std of residuals
→ rank XS → long-low-IVOL (canonical anomaly direction).

Both signs tested for completeness.
...
# ── Idiosyncratic vol panel (rolling CAPM residual std) ─────────────────────
def compute_ivol(rets: pd.DataFrame, bench: str = BENCHMARK,
    ...
    for t in rets.columns:
        ...
        resid = r - alpha - beta * spy
        out[t] = resid.rolling(beta_window).std()
    return out
...
def ivol_sleeve(rets: pd.DataFrame, feats: dict, ivol: pd.DataFrame,
                  candidates: list, sign: int = -1,
                  top_k: int = 5, sleeve_pct: float = 0.10,
                  cap_per_name: float = 0.05,
                  sizing: str = "atr") -> pd.DataFrame:
    """sign=-1 (canonical): long lowest IVOL."""
    cands = [t for t in candidates if t in ivol.columns]
    sub = ivol[cands] * sign
...
def main():
    ...
    print("\nComputing rolling-60d/120d IVOL...")
    t1 = time.time()
    ivol60  = compute_ivol(rets, BENCHMARK, beta_window=60)
    ivol120 = compute_ivol(rets, BENCHMARK, beta_window=120)
    ...
    base_m = report("V4N base (no IVOL sleeve)", base, rets)
    if base_m is None:
        return
    ...
    # ── Phase 1: STANDALONE ──────────────────────────────────────────────────
    print("\n--- Phase 1. IVOL sleeve STANDALONE ---")
    print(f"{'Variant':<70} {'Sh':>5} {'Ann':>7} {'DD':>7} {'Cal':>5} {'Vol':>6}")
    for univ_name, univ in [("stocks", stocks), ("full", full)]:
        for ivol_name, ivol in [("iv60", ivol60), ("iv120", ivol120)]:
            for sn in (-1, +1):
                for k in (3, 5, 8):
                    sl = ivol_sleeve(rets, feats, ivol, univ,
                                       sign=sn, top_k=k, sleeve_pct=1.0,
                                       cap_per_name=0.20)
                    sn_tag = "+1" if sn == +1 else "-1"
                    m = report(f"P1: {univ_name} {ivol_name} sgn{sn_tag} k{k}",
                                sl, rets)
            ...
            for sn in (-1, +1):
                for k in (3, 5):
                    for sp in (0.05, 0.08, 0.12):
                        ...
                        sn_tag = "+1" if sn == +1 else "-1"
                        m = report(f"P2: {univ_name} {ivol_name} sgn{sn_tag} k{k} sp{sp}",
                ...
                for k in (3, 5):
                    for sp in (0.05, 0.08):
                        for mg in (1.05, 1.10):
                            ...
                            sn_tag = "+1" if sn == +1 else "-1"
                            m = report(f"P3: {univ_name} {ivol_name} sgn{sn_tag} k{k} sp{sp} g{mg}",
                                        combined, rets, base_dd, base_ann, base_pr_oos)
