"""
Cross-sectional sector-rotation momentum sleeve on top of V4N-F base.

Hypothesis: At the sector level (XLK, XLF, XLV, etc.), there's a XS momentum
effect distinct from the single-name residual momentum we already have.  V4N's
ADX/trend filter picks individual names; a sector-rotation sleeve picks the
top-K sectors by trailing N-month return and longs sector ETFs.

Reverse hypothesis: At sector granularity, *short-term reversal* (1m or 1w)
may work better than 12-1 momentum (Jegadeesh-Titman classical effect at name
level may invert at sector level due to crowding).  Both signs tested.

Signals (per sector ETF):
  * mom_w  — trailing N-day return (windows: 21, 63, 126, 252)
  * vol_adj_mom — mom_w / rolling_vol_w (Sharpe-like)
  * rev_w  — short-term reversal (5d, 10d) — long bottom

Universe: 9 sector ETFs: XLE, XLU, XLF, VNQ, XLK, XLV, XLI, XLC, XLP.
...
def build_sector_panel(rets: pd.DataFrame, sectors: list,
                         kind: str, window: int) -> pd.DataFrame:
    out = pd.DataFrame(index=rets.index, columns=sectors, dtype=float)
    for s in sectors:
        if s not in rets.columns:
            continue
        r = rets[s]
        if kind == "mom":
            out[s] = r.rolling(window).sum()
        elif kind == "vol_adj_mom":
            mom = r.rolling(window).sum()
            vol = r.rolling(window).std().replace(0, np.nan)
            out[s] = mom / vol
        elif kind == "rev":
            out[s] = r.rolling(window).sum()  # invert via sign later
        else:
            raise ValueError(kind)
    return out


def sector_sleeve(rets: pd.DataFrame, feats: dict, panel: pd.DataFrame,
                    sign: int = +1, top_k: int = 3, short_bottom_k: int = 0,
                    sleeve_pct: float = 0.10, cap_per_name: float = 0.05,
...
def main():
    ...
    for kind, windows in [("mom", (21, 63, 126, 252)),
                           ...
                           ("rev", (5, 10, 21))]:
        for w in windows:
            panels[f"{kind}_w{w}"] = build_sector_panel(rets, sectors, kind, w)
    ...
    base_m = report("V4N base (no sector-rot sleeve)", base, rets)
    if base_m is None:
        return
    ...
    # ── Phase 1: STANDALONE ──────────────────────────────────────────────────
    print("\n--- Phase 1. Sector-rotation sleeve STANDALONE ---")
    print(f"{'Variant':<70} {'Sh':>5} {'Ann':>7} {'DD':>7} {'Cal':>5} {'Vol':>6}")
    p1_panels = ["mom_w63", "mom_w126", "mom_w252", "vol_adj_mom_w126",
                 "rev_w5", "rev_w21"]
    for pkey in p1_panels:
        for sn in (+1, -1):
            for k in (2, 3, 5):
                sl = sector_sleeve(rets, feats, panels[pkey],
                                     sign=sn, top_k=k, sleeve_pct=1.0,
                                     cap_per_name=0.25)
                sn_tag = "+1" if sn == +1 else "-1"
                m = report(f"P1: {pkey} sgn{sn_tag} k{k}", sl, rets)
                if m: results.append(m | {"_phase": "p1"})

    # Long/short variants
    print("\n--- Phase 1b. Long/short sector rotation ---")
    for pkey in ("mom_w63", "mom_w126", "mom_w252"):
        for k in (2, 3):
            sl = sector_sleeve(rets, feats, panels[pkey],
                                 sign=+1, top_k=k, short_bottom_k=k,
                                 sleeve_pct=1.0, cap_per_name=0.15)
            m = report(f"P1b: {pkey} L/S k{k}", sl, rets)
        ...
        for sn in (+1, -1):
            for k in (2, 3):
                for sp in (0.05, 0.08, 0.12):
                    sl = sector_sleeve(rets, feats, panel,
                                         sign=sn, top_k=k, sleeve_pct=sp,
                                         cap_per_name=0.05)
                    combined = _layer_long_sleeve(base, sl, sp)
                    sn_tag = "+1" if sn == +1 else "-1"
                    m = report(f"P2: {pkey} sgn{sn_tag} k{k} sp{sp}",
            ...
            for k in (2, 3):
                for sp in (0.05, 0.08):
                    for mg in (1.05, 1.10):
                        ...
                        sn_tag = "+1" if sn == +1 else "-1"
                        m = report(f"P3: {pkey} sgn{sn_tag} k{k} sp{sp} g{mg}",
                                    combined, rets, base_dd, base_ann, base_pr_oos)
